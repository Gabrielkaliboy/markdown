 ...
 */
 //参考于https://github.com/chokcoco/jQuery-/blob/master/jquery-1.10.2__read.js
...
var
	...
	//所有jQuery对象最后的指向应该都是回到jQuery(document)
	rootjQuery,

	// Support: IE<10
	// For `typeof xmlNode.method` instead of `xmlNode.method !== undefined`
	//将undefined转换为字符串undefined，typeof会返回我们对象的类型,并且对象类型标注为字符串格式
	//typeof undefined--->"undefined"    typeof Array---->"function"   typeof 222--->"number"
	core_strundefined = typeof undefined,

	// Use the correct document accordingly with window argument (sandbox)
	//accordingly:因此，于是；相应的，照着
	//通过闭包函数传入window对象，避免document之类的全局变量被其他插件修改
	location = window.location,
	document = window.document,
	docElem = document.documentElement,

	// Map over jQuery in case of overwrite
	//设置别名，通过两个私有变量映射window环境下的jQuery和$两个对象，以防止变量被强行覆盖
	...
	completed = function( event ) {
		...
		//兼容低版本IE，当状态码为complete的时候，dom就准备好了
	...
	detach = function() {
		...
		//Opera 7.0 及 Opera 更早版本也不支持。 但是，对于这些不支持该函数的浏览器，
		//你可以使用 attachEvent() 方法来添加事件句柄。
		if ( document.addEventListener ) {
			//移除 addEventListener() 方法添加的事件:
			document.removeEventListener( "DOMContentLoaded", completed, false );
			//document和window都是对象element.removeEventListener
			window.removeEventListener( "load", completed, false );
			//Internet Explorer 8 及更早IE版本不支持 removeEventListener() 方法，
			//Opera 7.0 及 Opera 更早版本也不支持。 但是，对于这些不支持该函数的浏览器，
			//你可以使用 detachEvent() 方法来移除由 attachEvent() 方法添加的事件句柄。
	...
	init: function( selector, context, rootjQuery ) {
		...
		if ( typeof selector === "string" ) {
			...
			if ( selector.charAt(0) === "<" && selector.charAt( selector.length - 1 ) === ">" && selector.length >= 3 ) {
				...
				//rquickExpr上面有,检测HTML字符串的表达式。exec是一个方法,返回一个数组，其中存放匹配的结果。如果未找到匹配，则返回值为 null。
				//exec:http://www.w3school.com.cn/jsref/jsref_exec_regexp.asp
